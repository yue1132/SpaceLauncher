...
> 🚀 **macOS 上最快启动应用的方式** — 按住触发键，按一个键，完成。

一款轻量级 macOS 应用启动器，将**你选择的键变为修饰键**。不再需要 `Cmd+空格` 或伸手去拿鼠标 — 只要按住触发键，轻按一个键就能瞬间启动任何应用。支持 **空格**、**大写锁定（Caps Lock）** 和 **ESC** 作为触发键。

## ✨ 核心特色

| 功能 | 说明 |
|------|------|
| 🎯 **可配置触发键** | 选择空格、Caps Lock 或 ESC 作为触发键 — 按住即激活 |
| ⚡ **瞬间启动** | 无 UI 延迟，无需搜索 — 肌肉记忆即可 |
| 🔄 **连续切换** | 保持按住触发键，可连续按多个键快速切换应用 |
...
1. 启动 SpaceLauncher
2. **授予辅助功能权限**（按引导提示操作）
3. 按住触发键（默认空格）约 150ms，直到出现提示面板
4. 保持按住触发键，按下配置的键启动应用
5. 松开触发键，面板消失
...
| 技巧 | 说明 |
|------|------|
| 🔄 **连续切换** | 保持按住触发键，连续按多个键可快速在应用间切换 |
| ⏸️ **暂停功能** | 玩游戏时点击菜单栏图标 →「暂停运行」 |
| 🔄 **重载配置** | 修改配置后，右键菜单栏图标 →「重新加载配置」 |
| 🔐 **开机自启** | 从菜单栏启用「开机自启」 |
| 🎹 **更换触发键** | 在配置文件中设置 `trigger_key` 为 `space`、`caps_lock` 或 `esc` |
...
```yaml
settings:
  hold_threshold_ms: 150  # 按住触发键激活的阈值时间(毫秒)
  trigger_key: space      # 触发键: space, caps_lock, 或 esc
...
SpaceLauncher 需要监听全局键盘事件来检测触发键，这需要辅助功能权限。应用只在触发键被按下时监听键盘事件，不会记录或存储任何按键信息。
...
可以！在配置文件中设置 `trigger_key` 即可。可用选项：

| 按键 | 配置值 | 说明 |
|------|--------|------|
| 空格 | `space` | 默认值。短按仍然正常输入空格 |
| 大写锁定 | `caps_lock` | 适合键盘爱好者 |
| ESC | `esc` | 较少使用，冲突更少 |

## 🔮 开发计划

- [x] 自定义触发键支持（空格、Caps Lock、ESC）
- [ ] 提示面板显示应用图标
- [ ] 多键组合快捷键
- [ ] URL Scheme 支持
- [ ] GitHub 自动更新
